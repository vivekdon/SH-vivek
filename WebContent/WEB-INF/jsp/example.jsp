...
<script>
    ...
    function drawTechanChart(){
	 ...
	 success: function(json) {
         ...
         indicatorPreRoll = 33;  // Don't show where indicators don't have data
		 	x.domain(techan.scale.plot.time(chartData).domain());
	        y.domain(techan.scale.plot.ohlc(chartData.slice(indicatorPreRoll)).domain());
	        yPercent.domain(techan.scale.plot.percent(y, accessor(chartData[indicatorPreRoll])).domain());
	        yVolume.domain(techan.scale.plot.volume(chartData).domain());
			
	        var macdData = techan.indicator.macd()(chartData);
	        macdScale.domain(techan.scale.plot.macd(macdData).domain());
	        var rsiData = techan.indicator.rsi()(chartData);
	        rsiScale.domain(techan.scale.plot.rsi(rsiData).domain());

	        svg.select("g.candlestick").datum(chartData).call(candlestick);
	        svg.select("g.close.annotation").datum([chartData[chartData.length-1]]).call(closeAnnotation);
	        svg.select("g.volume").datum(chartData).call(volume);
	        svg.select("g.sma.ma-0").datum(techan.indicator.sma().period(10)(chartData)).call(sma0);
	        svg.select("g.sma.ma-1").datum(techan.indicator.sma().period(20)(chartData)).call(sma1);
	        svg.select("g.ema.ma-2").datum(techan.indicator.ema().period(50)(chartData)).call(ema2);
	        ...
	        // Stash for zooming
	        zoomableInit = x.zoomable().domain([indicatorPreRoll, chartData.length]).copy(); // Zoom in a little to hide indicator preroll
	        yInit = y.copy();
    ...
    /*	
    d3.csv("http://localhost:8080/SH/data.csv", function(error, data) { 
        var accessor = candlestick.accessor(),
            indicatorPreRoll = 33;  // Don't show where indicators don't have data
